...
class AllianzFormData(BaseModel):
    ...
    address_street: str = Field(..., description="Street, city")
    address_state: str = Field(..., description="State")
    address_postcode: str = Field(..., description="Postcode")
    telephone_work: str = Field(..., description="Work telephone")
    telephone_home: str = Field(..., description="Home telephone")
    telephone_mobile: str = Field(..., description="Mobile telephone")
    email: str = Field(..., description="Email")
    date_of_birth: date = Field(..., description="Date of birth")
    country_of_birth: str = Field(..., description="Country of birth")
    language: str = Field(..., description="Language")
    is_interpreter_required: bool = Field(..., description="Is an interpreter required?")
    temporarily_in_australia_on_visa: bool = Field(
        ..., description="Are you temporarily in Australia on a visa?"
    )
    expiry_date_of_visa: date = Field(..., description="Expiry date of visa if applicable")
    visa_type: str = Field(..., description="Type of visa if applicable")
    marital_status: str = Field(..., description="Marital status")
    dependent_details: list[dict[str, str | date]] = Field(
        ..., description="Name, relationship, and date of birth"
    )
    how_did_injury_occur: str = Field(..., description="How did the injury occur?")
    what_were_you_doing_when_injury_happened: str = Field(
        ..., description="Example: slipped when climbing a ladder"
    )
    parts_of_body_injured: str = Field(..., description="Part(s) of body injured")
    was_body_part_fully_functional_before_injury: bool = Field(
        ..., description="If no, provide details"
    )
    body_part_details_before_injury: str = Field(
        ..., description="Details of any pre-existing conditions"
    )
    address_where_injury_happened_street_city: str = Field(
        ..., description="If different to work address"
    )
    address_where_injury_happened_state: str = Field(
        ..., description="If different to work address"
    )
    address_where_injury_happened_postcode: str = Field(
        ..., description="If different to work address"
    )
    date_of_injury: date = Field(..., description="Date of injury")
    time_of_injury: time = Field(..., description="AM or PM")
    did_anyone_see_injury_occur: bool = Field(..., description="If yes, provide witness names")
    names_of_witnesses: list[str] = Field(..., description="Names of witnesses")
    person_at_workplace_reported_injury_to_name: str = Field(..., description="Name")
    person_at_workplace_reported_injury_to_job_title: str = Field(..., description="Job title")
    person_at_workplace_reported_injury_to_date_reported: date = Field(
        ..., description="Date reported"
    )
    nominated_treating_doctor_name: str = Field(..., description="Name")
    nominated_treating_doctor_telephone: str = Field(..., description="Telephone")
    previous_similar_injuries: bool = Field(..., description="If yes, provide details")
    previous_injury_details: str = Field(..., description="Details of previous similar injuries")
    do_you_have_second_job: bool = Field(
        ..., description="Name of second employer, contact name, telephone"
    )
    name_of_second_employer: str = Field(..., description="Name of second employer, if applicable")
    contact_name_of_second_employer: str = Field(
        ..., description="Contact name of second employer, if applicable"
    )
    telephone_of_second_employer: str = Field(
        ..., description="Contact telephone of second employer, if applicable"
    )
    average_weekly_earnings_from_job: float = Field(
        ..., description="Average weekly earnings from this job"
    )
    average_weekly_hours_from_job: float = Field(
        ..., description="Average weekly hours from this job"
    )
    declaration_name: str = Field(
        ..., description="Worker's name for confirmation that information is correct"
    )
    declaration_date: date = Field(
        ..., description="Date for confirmation that information is correct"
    )
    authority_name: str = Field(
        ..., description="Worker's name to authorise release of medical/factual history"
    )
    injury_sustained_on: date = Field(..., description="Date of injury")
    authority_date: date = Field(
        ..., description="Date for authorisation of release of medical/factual history"
    )
    date_form_provided_to_employer: date = Field(
        ..., description="Date form was provided to employer"
    )
    received_by_employer_name: str = Field(..., description="Name of person who received the form")
    received_by_employer_job_title: str = Field(
        ..., description="Job title of person who received the form"
    )
    date_received_by_employer: date = Field(..., description="Date employer received the form")
    additional_information: str = Field(..., description="From either worker or employer")


class DWCFormData(BaseModel):
    ...
    date_of_injury: date = Field(..., description="Date when accident or illness occurred")
    ...
    name_of_employer: str = Field(..., description="Name of employer")
...
class WorkSafeFormData(BaseModel):
    ...
    number_of_children: int = Field(..., description="Number of children, if applicable")
    ...
    other_paid_employment: bool = Field(..., description="If yes, provide employer details")
    ...
    exact_location_of_occurrence: str = Field(..., description="Exact location of occurrence")
    ...
    name_person_reported_to: str = Field(..., description="Name of the person reported to")
    position_person_reported_to: str = Field(..., description="Position of the person reported to")
    ...
    time_started_shift: time = Field(..., description="If yes, provide time started shift")
    ...
    still_receiving_treatment: bool = Field(..., description="Yes/No, if yes provide details")
    ...
    claiming_for: str = Field(..., description="Time off work, medical expenses, rehab, hospital")
    ...
    type_injury_disease_before: str = Field(..., description="Type of injury/disease before")
    date_injury_disease_before: date = Field(..., description="Date of injury/disease before")
    ...
    employer_business_trading_name: str = Field(..., description="Business trading name")
    employer_abn: str = Field(..., description="Australian business number")
    employer_acn: str = Field(..., description="Australian company number, if applicable")
    ...
    employer_contact_person_position: str = Field(..., description="Contact person position")
    ...
    workers_compensation_policy_expiry_date: date = Field(..., description="Policy expiry date")
    ...
    injured_worker_start_date: date = Field(..., description="Worker employment start date")
    injured_worker_a_contractor: bool = Field(..., description="Is the worker a contractor?")
