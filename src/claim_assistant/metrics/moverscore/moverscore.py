...
def load_ngram(ids, embedding, idf, n, o):
    ...
    for slide_win in slide_wins:
        ...
        scale = (
            _safe_divide(idf[slide_win], idf[slide_win].sum(0)).unsqueeze(-1).to(device)
        )
...
def word_mover_score(
    ...
    for batch_start in range(0, len(refs), batch_size):
        ...
        ref_embedding = torch.cat(
            [ref_embedding_min, ref_embedding_avg, ref_embedding_max],
            -1,
        )
        hyp_embedding = torch.cat(
            [hyp_embedding_min, hyp_embedding_avg, hyp_embedding_max],
            -1,
        )
